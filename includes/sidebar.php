...
<style>
  ...
  .nav-sidebar .nav-link {
    color: #d1d5db;
    border-radius: 4px;
    margin: 0.05em 0.4em 0.05em 1.2em;
    padding: 0.4em 0.8em;
...
<aside class="main-sidebar sidebar-dark-primary elevation-4">
  ...
  <div class="sidebar">
    <?php 
    // First, render Dashboard items as standalone (not in a group)
    ...
    foreach ($menu as $group => $items): 
      ...
      $group_icons = [
        ...
        'System' => 'fas fa-cogs',
        'Administration' => 'fas fa-shield-alt',
        'Health' => 'fas fa-heartbeat',
        'Events' => 'fas fa-calendar-alt',
        'Bulk Operations' => 'fas fa-layer-group',
...
function toggleMenuGroup(groupId, headerElement) {
  ...
  if (isCollapsed) {
    ...
    const allHeaders = document.querySelectorAll('.sidebar-heading');
    ...
    // Now expand the clicked group
    headerElement.classList.remove('collapsed');
    menuGroup.style.maxHeight = menuGroup.scrollHeight + 'px';
    menuGroup.classList.remove('collapsed');
    
    // Store which group is open (only one at a time)
    localStorage.setItem('sidebar-open-group', groupId);
  } else {
    ...
    // Clear the stored open group
    localStorage.removeItem('sidebar-open-group');
  }
}

// Restore menu state on page load (only one group open)
document.addEventListener('DOMContentLoaded', function() {
  const menuGroups = document.querySelectorAll('.menu-group');
  const menuHeaders = document.querySelectorAll('.sidebar-heading');
  const openGroupId = localStorage.getItem('sidebar-open-group');
  ...
  // Then expand only the saved open group (or Dashboard by default)
  let groupToOpen = null;
  if (openGroupId) {
    groupToOpen = document.getElementById(openGroupId);
  } else {
    // Default to opening Dashboard group if no saved state
    groupToOpen = document.getElementById('menu-group-dashboard');
  }
  
  if (groupToOpen) {
    const header = groupToOpen.previousElementSibling;
    header.classList.remove('collapsed');
    groupToOpen.style.maxHeight = groupToOpen.scrollHeight + 'px';
    groupToOpen.classList.remove('collapsed');
    
    // Save the default open group
    if (!openGroupId) {
      localStorage.setItem('sidebar-open-group', groupToOpen.id);
    }
  }
});
</script>
