...
$selectedClassId = (int) ($_REQUEST['class_id'] ?? array_key_first($classMap));
if (!isset($classMap[$selectedClassId])) {
    http_response_code(403);
    include __DIR__ . '/errors/403.php';
    exit;
}
$selectedClass = $classMap[$selectedClassId];
$error = '';
        ...
        try {
            ...
            if ($action === 'request') {
                ...
                header('Location: bible_class_member_removals.php?' . http_build_query([
                    'class_id' => $selectedClassId, 'message' => 'Removal request submitted for administrator review.',
                ]));
                exit;
            }
            if ($action === 'review') {
                ...
                header('Location: bible_class_member_removals.php?' . http_build_query([
                    'class_id' => $selectedClassId, 'message' => 'Removal request reviewed.',
                ]));
                exit;
            }
...
$members = [];
if ($canRequest && $scheduleService->canAccessClass($selectedClassId)) {
    $members = $scheduleService->getActiveMembers($selectedClassId);
}
$preselectedMemberId = (int) ($_GET['member_id'] ?? 0);
$requests = $operationsService->listRequests($selectedClassId, $canReview);
...
<div class="container-fluid py-4">
    <div class="d-flex flex-wrap justify-content-between align-items-center mb-4">
        ...
        <a class="btn btn-secondary" href="bible_class_book.php?class_id=<?= $selectedClassId ?>"><i class="fas fa-arrow-left mr-1"></i>Class Book</a>
    </div>
    <?php if ($error): ?><div class="alert alert-danger"><?= htmlspecialchars($error) ?></div><?php endif; ?>
    <?php if ($success): ?><div class="alert alert-success"><?= htmlspecialchars($success) ?></div><?php endif; ?>

    <div class="card shadow-sm mb-4"><div class="card-body"><form method="get" class="form-row align-items-end"><div class="form-group col-md-8 mb-md-0"><label for="class_id">Bible class</label><select class="form-control" id="class_id" name="class_id" onchange="this.form.submit()">
    ...
    <div class="card shadow-sm mb-4"><div class="card-header bg-white"><strong>Request removal from <?= htmlspecialchars($selectedClass['name']) ?></strong></div><div class="card-body"><form method="post">
        <?= csrf_input() ?><input type="hidden" name="action" value="request"><input type="hidden" name="class_id" value="<?= $selectedClassId ?>">
