...
if (!$is_super_admin && !has_permission('view_age_bracket_payment_report')) {
...
$can_export = $is_super_admin || has_permission('export_age_bracket_payment_report');
...
$selected_bracket = isset($_GET['age_bracket']) ? $_GET['age_bracket'] : '';
$payment_types = [];
...
if ($selected_bracket && isset($age_brackets[$selected_bracket])) {
    $min_age = $age_brackets[$selected_bracket][0];
    $max_age = $age_brackets[$selected_bracket][1];
    $where[] = "TIMESTAMPDIFF(YEAR, m.dob, CURDATE()) BETWEEN $min_age AND $max_age";
}
...
function get_age_bracket($dob, $brackets) {
    if (!$dob) return '-';
    $age = date_diff(date_create($dob), date_create('today'))->y;
    foreach ($brackets as $label => $range) {
        if ($age >= $range[0] && $age <= $range[1]) return $label;
    }
    return '61+';
}
?>
<div class="container mt-4">
    ...
    <h2 class="mb-4 font-weight-bold"><i class="fas fa-chart-bar mr-2"></i>Age Bracket Payment Report</h2>
    <form method="get" class="form-inline mb-3">
        <div class="form-group mr-2">
            <label for="age_bracket" class="mr-2 font-weight-bold">Age Bracket:</label>
            <select name="age_bracket" id="age_bracket" class="form-control">
                <option value="">All</option>
                <?php foreach ($age_brackets as $label => $range): ?>
                    <option value="<?php echo $label; ?>"<?php if ($selected_bracket === $label) echo ' selected'; ?>><?php echo htmlspecialchars($label); ?></option>
                <?php endforeach; ?>
            </select>
        </div>
        <div class="form-group mr-2">
                ...
                <?php else: ?>
                    <?php foreach ($rows as $i => $row): ?>
                        <tr>
                            ...
                            <td><?php echo htmlspecialchars(get_age_bracket($row['dob'], $age_brackets)); ?></td>
