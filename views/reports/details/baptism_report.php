...
if (!$is_super_admin && !has_permission('view_baptism_report')) {
...
$can_export = $is_super_admin || has_permission('export_baptism_report');
...
$conn = $GLOBALS['conn'];
$baptized_options = ['Yes', 'No'];
$selected_baptized = isset($_GET['baptized']) ? $_GET['baptized'] : '';

$sql = "SELECT m.crn, m.last_name, m.first_name, m.baptized, m.date_of_baptism, m.gender, m.phone, m.dob, m.home_town FROM members m WHERE m.status = 'active'";
if ($selected_baptized === 'Yes') {
    $sql .= " AND m.baptized = 'Yes'";
} elseif ($selected_baptized === 'No') {
    $sql .= " AND (m.baptized = 'No' OR m.baptized IS NULL OR m.baptized = '')";
}
$sql .= " ORDER BY m.baptized DESC, m.last_name, m.first_name";
...
<div class="container mt-4">
    ...
    <h2 class="mb-4 font-weight-bold"><i class="fas fa-tint mr-2"></i>Baptism Report</h2>
    <form method="get" class="form-inline mb-3">
        <div class="form-group mr-2">
            <label for="baptized" class="mr-2 font-weight-bold">Filter by Baptized:</label>
            <select name="baptized" id="baptized" class="form-control">
                <option value="">All</option>
                <?php foreach ($baptized_options as $opt): ?>
                    <option value="<?php echo $opt; ?>"<?php if ($selected_baptized === $opt) echo ' selected'; ?>><?php echo $opt; ?></option>
                <?php endforeach; ?>
            </select>
        </div>
                ...
                <?php else: ?>
                    <?php foreach ($members as $i => $member): ?>
                        <tr>
                            ...
                            <td><?php echo htmlspecialchars($member['date_of_baptism'] ?: '-'); ?></td>
