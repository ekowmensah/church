...
// Query: Group by payment period
$sql = "SELECT 
    COALESCE(p.payment_period_description, DATE_FORMAT(p.payment_date, '%Y-%m')) AS period,
    COALESCE(p.payment_period, DATE_FORMAT(p.payment_date, '%Y-%m-01')) AS period_date,
...
$where
GROUP BY period_date, pt.id
ORDER BY period_date DESC, pt.name";
...
// Generate filename
$filename = 'payment_by_period_' . date('Y-m-d_H-i-s') . '.csv';
...
// CSV Headers
fputcsv($output, ['Period', 'Payment Type', 'Number of Payments', 'Total Amount (₵)']);
